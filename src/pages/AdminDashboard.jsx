...
function AdminDashboard() {
  ...
  const fetchPrayerRequests = async () => {
    try {
      ...
      setError(error.message || "Unable to fetch prayer requests.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMarkAsPrayed = async (prayerId) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.detail || "Unable to update prayer status");
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <p className="text-xl text-gray-400">Loading prayer requests...</p>
      </div>
    );
  }
      ...
      {prayerRequests.filter(
        ...
        {prayerRequests.length === 0 ? (
          <div className="bg-[#111827] border border-gray-800 rounded-2xl p-10 text-center">
            <p className="text-gray-400 text-lg">No prayer requests found.</p>
          </div>
        ) : (
          /* REQUEST LIST */
            ...
            {filteredPrayerRequests.map((prayer) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold">{prayer.name}</h2>
                  ...
                  >
                    {prayer.status === "prayed" ? "PRAYED" : "NEW"}
                  </span>
                </div>

                {/* PHONE */}
                <p className="text-gray-400 mt-3">📞 {prayer.phone}</p>

                {/* PRAYER */}
                <div className="mt-5">
                  <p className="text-gray-300 leading-7">{prayer.prayer}</p>
                </div>

                {/* DATE */}
                {prayer.created_at && (
                  <p className="text-gray-500 text-sm mt-5">
                    Submitted: {new Date(prayer.created_at).toLocaleString()}
                  </p>
                )}

                {/* ACTION */}
                {prayer.status === "new" &&
                  ["admin", "prayer"].includes(adminUser?.role) && (
                    <button
